...
class App {
  ...
  setUpModel() {
    ...
    const torusKnotGeometry = new TorusKnotGeometry(1, 0.3, 128, 64, 2, 3);
    const bigSphereMaterial = new MeshStandardMaterial({
      color: "#ffffff",
      roughness: 0.1,
      metalness: 0.2,
    });
    const centerTorusKnot = new Mesh(torusKnotGeometry, bigSphereMaterial);
    // bigSphere.rotation.x = MathUtils.degToRad(-90);
    centerTorusKnot.position.y = 1.6;
    centerTorusKnot.castShadow = true;
    centerTorusKnot.receiveShadow = true;

    this.scene.add(centerTorusKnot);
  ...
  update(time: number) {
    ...
    if (this.light instanceof DirectionalLight && this.light?.target) {
